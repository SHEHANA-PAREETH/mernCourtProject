...
console.log(bookingData);
    ...
    const cancelBooking=()=>{
      AxiosInstance.get('payment/cancelBooking',{params:{shedulesid:bookingData._id}}).then((resp)=>{
        console.log(resp);
        if(resp.data.data){
          alert("cancelled successfully ,amount refunded successfully check your wallet")
  ...
  <div className="m-5 card-container">
        <a href="#" className="hero-image-container">
          <img src={`${BASEURL}/uploads/${bookingData?.courts?.image[0]?.filename}`}/>
        </a>
        <main class="main-content">
          <h1><a href="#">{bookingData.courts.name}</a></h1>
